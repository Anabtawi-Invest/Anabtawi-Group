...
class HrPayslipRun(models.Model):
    ...
    def _generate_payrun_xlsx(self, quick_audit=False):
        ...
        # Write Provenance / Metadata (Rows 0-5)
        ...
        meta_items = [
            ...
            (_("Template Version"), "v1.0 (Audit Enabled)"),
        ]
        ...
        columns = [
            ...
            (_("Salary Adjustments"), 18),
            (_("Net Salary"), 18),
            (_("Attendance Days"), 16),
            (_("Worked Hours"), 15),
            (_("Overtime Hours"), 15),
            (_("Note / Description"), 25),
        ]
        ...
        if not payslips:
            ...
            for payslip in payslips:
                ...
                emp_id_val = str(emp.id) if emp else "N/A"
                if not emp:
                ...
                if emp and code_val == "N/A":
                    issues.append(_("Missing Employee Code"))
                ...
                salary_adj = get_line_amt(["ADJ", "SALARY_ADJ", "BULK_ADJ"])

                # 12. Net Salary
                net_sal = payslip.net_wage if hasattr(payslip, "net_wage") else get_line_amt(["NET"], "NET")
                if not net_sal and gross_sal:
                    net_sal = gross_sal - tax_val - ssce_val - loan_val - other_ded + salary_adj
                ...
                note_val = payslip.note or payslip.name or ""
                ...
                sheet1.write_number(data_row, 17, salary_adj, number_fmt)

                net_style = net_negative_fmt if net_sal < 0 else number_fmt
                sheet1.write_number(data_row, 18, net_sal, net_style)

                sheet1.write_number(data_row, 19, att_days, int_fmt)
                sheet1.write_number(data_row, 20, worked_hrs, number_fmt)
                sheet1.write_number(data_row, 21, ot_hrs, number_fmt)
                sheet1.write(data_row, 22, note_val, text_left_fmt)
