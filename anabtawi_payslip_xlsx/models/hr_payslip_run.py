...
class HrPayslipRun(models.Model):
    ...
    def _generate_payrun_xlsx(self, quick_audit=False, payslip_ids=None):
        ...
        meta_items = [
            ...
            (_("Template Version"), "v1.3 (Strict Payslip Line Mirroring)"),
        ]
        ...
        total_gross_val = sum(payslips.mapped("gross_wage") if "gross_wage" in payslips._fields else [
            sum(p.line_ids.filtered(lambda l: l.code == "GROSS" or l.category_id.code == "GROSS").mapped("total")) for p in payslips
        ])
        total_net_val = sum(payslips.mapped("net_wage") if "net_wage" in payslips._fields else [
            sum(p.line_ids.filtered(lambda l: l.code == "NET" or l.category_id.code == "NET").mapped("total")) for p in payslips
        ])
        
        # Calculate summary metrics directly from payslip lines
        ...
        if not payslips:
            ...
            for payslip in payslips:
                ...
                # Line helper strictly extracting from payslip.line_ids
                ...
                # 1. Basic Salary (sum of lines under BASIC category or code)
                basic_sal = sum(lines.filtered(lambda l: l.category_id.code == "BASIC" or l.code == "BASIC").mapped("total"))

                # 2. Remaining Leaves Compensation
                rem_leave = get_line_amt(["REM_LEAVE", "LEAVE_COMP", "ANNUAL_LEAVE"])
                
                # 3. Gross Attendance / Overtime
                gross_att_ot = get_line_amt(["OVERTIME", "GROSS_ATT", "OT_COMP", "EXTRA_HOURS"])

                # 4. Other Earnings (all ALW category lines not captured in basic, leaves, overtime)
                ...
                # 5. Gross Salary directly from payslip line or gross_wage
                gross_sal = payslip.gross_wage if hasattr(payslip, "gross_wage") and payslip.gross_wage else get_line_amt(["GROSS"], "GROSS")
                if not gross_sal:
                ...
                # 11. Other Deductions (DED category lines minus tax, ssce, loan, and input deductions)
                ...
                # 12. Net Salary directly from payslip.net_wage
                net_sal = payslip.net_wage if hasattr(payslip, "net_wage") and payslip.net_wage else get_line_amt(["NET"], "NET")
