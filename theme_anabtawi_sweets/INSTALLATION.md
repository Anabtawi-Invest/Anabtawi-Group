...
2. Copy the `theme_anabtawi_sweets` directory into a directory listed in Odoo's `addons_path`.
3. Restart Odoo.
4. Enable developer mode, open **Apps**, and select **Update Apps List**.
5. Remove the default **Apps** filter if necessary, search for **Anabtawi Sweets Website Theme**, and install it.
...
```text
odoo-bin -d DATABASE_NAME -i theme_anabtawi_sweets --stop-after-init
```

For a later code update:

```text
odoo-bin -d DATABASE_NAME -u theme_anabtawi_sweets --stop-after-init
```
