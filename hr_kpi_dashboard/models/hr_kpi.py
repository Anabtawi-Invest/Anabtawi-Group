...
class HrKpiPeriod(models.Model):
    ...
    def _compute_kpis(self):
        ...
        metrics = {
            ...
            "INTERNAL_MOBILITY_RATE": self._get_internal_mobility_rate(employee_count),
            "TRAINING_PARTICIPATION_RATE": self._get_training_participation_rate(employee_count),
            "GRIEVANCES": 0.0,
    ...
    def _get_internal_mobility_rate(self, employee_count):
        self.ensure_one()
        if not employee_count:
            return 0.0
        contracts = self._period_contracts()
        moved_employees = self.env["hr.employee"].browse()
        for contract in contracts:
            if contract.date_start and self.date_from <= contract.date_start <= self.date_to:
                prior_exists = self.env["hr.contract"].search_count([
                    ("employee_id", "=", contract.employee_id.id),
                    ("date_start", "<", contract.date_start),
                ])
                if prior_exists:
                    moved_employees |= contract.employee_id
        return (len(moved_employees) / employee_count) * 100.0

    def _get_training_participation_rate(self, employee_count):
        self.ensure_one()
        if not employee_count or "slide.channel.partner" not in self.env:
            return 0.0
        domain = [
            ("completion_date", ">=", self.date_from),
            ("completion_date", "<=", self.date_to),
        ]
        records = self.env["slide.channel.partner"].search(domain)
        completed_partners = records.mapped("partner_id")
        employees = self.env["hr.employee"].search([
            ("work_contact_id", "in", completed_partners.ids),
        ])
        return (len(employees) / employee_count) * 100.0
