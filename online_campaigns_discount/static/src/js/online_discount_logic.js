...
function taxExclusiveDiscountBase(line, taxIncludedDiscount, round) {
    ...
    return round(taxIncludedDiscount);
}
...
function normalizeRelationId(candidate) {
    return typeof candidate === "number" ? candidate : candidate?.id;
}

function campaignAppliesToCurrentConfig(campaign, posConfigId) {
    if (!campaign.pos_config_ids || !campaign.pos_config_ids.length) {
        return true;
    }
    return Boolean(posConfigId) && campaign.pos_config_ids.some(
        (candidate) => normalizeRelationId(candidate) === posConfigId
    );
}

function activePricelistCampaigns(campaigns, order, moment, posConfigId) {
    return campaigns
        .filter(
            (campaign) =>
                campaign.isActiveAt(moment) &&
                campaignAppliesToCurrentConfig(campaign, posConfigId) &&
                campaign.appliesToPricelist(order.pricelist_id)
        )
        .sort((left, right) => left.priority - right.priority || left.id - right.id);
}

function campaignCandidates(campaigns, order, product, moment, posConfigId) {
    const applicable = activePricelistCampaigns(campaigns, order, moment, posConfigId).filter((campaign) =>
...
function buildPerOrderAllocations(order, campaigns, moment, round, posConfigId) {
    ...
    for (const line of order.lines) {
        ...
        for (const campaign of campaignCandidates(campaigns, order, line.product_id, moment, posConfigId)) {
...
patch(PosStore.prototype, {
    ...
    recomputeOnlineCampaigns(order = this.getOrder()) {
        ...
        const perOrderAllocations = buildPerOrderAllocations(order, campaigns, moment, round, posConfigId);
        ...
        try {
            for (const line of order.lines) {
                ...
                if (refundedLine?.online_discount_amount) {
                    ...
                    const refundCommissionBase = commissionBaseAmount(
                        line, refundedLine.commission_base || "after_tax"
                    );
                    line.update({
                        ...
                        commission_base: refundedLine.commission_base || "after_tax",
                        aggregator_commission_amount: round(
                            (refundCommissionBase * refundedLine.aggregator_commission_percent) / 100
                        ),
                ...
                const selected = campaignCandidates(campaigns, order, line.product_id, moment, posConfigId);
                if (!selected.length) {
                    const channelCampaign = activePricelistCampaigns(campaigns, order, moment, posConfigId)[0];
                    if (channelCampaign) {
                        ...
                        const commissionBase = channelCampaign.aggregator_id?.commission_base || "after_tax";
                        const commissionBaseValue = commissionBaseAmount(line, commissionBase);
                        const values = {
                            ...emptyCampaignValues,
                            ...
                            aggregator_commission_amount: round(
                                (commissionBaseValue * channelCampaign.aggregator_commission_percent) / 100
                            ),
                        };
                        line.update(values);
                ...
                const rawTotal = breakdown.reduce((sum, item) => sum + item.discount_amount_incl_tax, 0);
                if (rawTotal > Math.abs(gross)) {
                    const factor = Math.abs(gross) / rawTotal;
                    breakdown = breakdown.map((item) => ({
                        ...item,
                        discount_amount_incl_tax: round(item.discount_amount_incl_tax * factor),
                    }));
                }
                const discountAmountInclTax = round(
                    breakdown.reduce((sum, item) => sum + item.discount_amount_incl_tax, 0)
                );
                const effectivePercent = Math.min(100, (discountAmountInclTax / Math.abs(gross)) * 100);
                ...
                const discountAmount = taxExclusiveDiscountBase(line, discountAmountInclTax, round);
                const ratioToTaxExclusive = discountAmountInclTax ? discountAmount / discountAmountInclTax : 0;
                breakdown = breakdown.map((item) => {
                    const baseAmount = round(item.discount_amount_incl_tax * ratioToTaxExclusive);
                    return {
                        ...item,
                        discount_amount: baseAmount,
                        aggregator_amount: round(
                            (baseAmount * selected.find((c) => c.id === item.campaign_id).aggregator_contribution_percent) / 100
                        ),
                    };
                });
                const aggregatorAmount = round(
                ...
                const commissionBase = primary.aggregator_id?.commission_base || "after_tax";
                const commissionBaseValue = commissionBaseAmount(line, commissionBase);
                line.update({
                    ...
                    commission_base: commissionBase,
                    aggregator_commission_amount: round(
                        (commissionBaseValue * primary.aggregator_commission_percent) / 100
                    ),
