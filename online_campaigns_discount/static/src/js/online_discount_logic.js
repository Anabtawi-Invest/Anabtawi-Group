...
function campaignAppliesToCurrentConfig(campaign, order) {
    const config = order.config_id || order.config || order.pos?.config;
    const configId = config?.id;
    if (!campaign.pos_config_ids || !campaign.pos_config_ids.length) {
        return true;
    }
    return Boolean(configId) && campaign.pos_config_ids.some((candidate) => candidate.id === configId);
}
    ...
    return campaigns
        .filter(
            (campaign) =>
                ...
                campaignAppliesToCurrentConfig(campaign, order) &&
