...
## 19.0.5.5.2
- Fixed real install blocker: `external_dependencies: {python: [openpyxl]}` in the manifest made Odoo mark the whole module "not installable, skipped" at the module-graph stage (before any code runs) because openpyxl wasn't present in this environment's Python env. Removed the declaration - the code already guards every openpyxl usage with try/except ImportError, so the module installs regardless; only Import/Export Statement would raise a clear error if openpyxl is genuinely missing. openpyxl still needs to be added to the Odoo.sh requirements.txt for that feature to actually work.
