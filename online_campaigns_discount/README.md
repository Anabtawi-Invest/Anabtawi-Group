...
```bash
odoo-bin -d DATABASE -i online_campaigns_discount --stop-after-init
```
...
```bash
odoo-bin -d TEST_DATABASE -i online_campaigns_discount \
  --test-enable --test-tags /online_campaigns_discount --stop-after-init
```
