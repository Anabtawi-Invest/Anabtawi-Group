...
class OnlineDiscountCampaign(models.Model):
    ...
    def action_finance_approve(self):
        ...
        if missing_accounts:
            raise UserError(_(
                "Configure the required aggregator receivable (for campaigns with aggregator contribution), company discount expense, and commission expense accounts before finance approval."
            ))
        for campaign in self.filtered(lambda item: item.state == "pending"):
            campaign.write({
