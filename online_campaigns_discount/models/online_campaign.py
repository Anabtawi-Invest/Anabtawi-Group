...
class OnlineDiscountCampaign(models.Model):
    ...
    aggregator_contribution_percent = fields.Float(
        string="Aggregator Discount Contribution %", required=True, default=0.0,
        digits=(16, 4), tracking=True
    )
    company_contribution_percent = fields.Float(
        string="Company Discount Contribution %", compute="_compute_company_contribution",
        store=True, readonly=True, digits=(16, 4), tracking=True
    )

    @api.depends("aggregator_contribution_percent")
    def _compute_company_contribution(self):
        for campaign in self:
            campaign.company_contribution_percent = 100.0 - campaign.aggregator_contribution_percent
    ...
    @api.constrains("aggregator_contribution_percent")
    def _check_contribution(self):
        for campaign in self:
            if not 0 <= campaign.aggregator_contribution_percent <= 100:
                raise ValidationError(_("Aggregator contribution percentage must be between 0 and 100%."))
    ...
    def action_finance_approve(self):
        ...
        if missing_accounts:
            raise UserError(_(
                "Configure the required aggregator receivable (for campaigns with aggregator contribution), company discount expense, campaign discount clearing, and commission expense accounts before finance approval."
            ))
        for campaign in self.filtered(lambda item: item.state == "pending"):
            campaign.write({
