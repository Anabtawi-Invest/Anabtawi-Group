...
class PosSession(models.Model):
    ...
    def _create_non_reconciliable_move_lines(self, data):
        ...
        - Both the aggregator's recoverable share and the company's own
          share are folded directly into each item's own core Sales line
          (topping up the credit core POS already posted for that product),
          rather than added as separate Sales lines. This grosses each
          item's Sales line back up to its full pre-discount, pre-tax price,
          proportional to that item's own discount - nothing hardcoded.
        - The aggregator's share is additionally debited to the aggregator
          receivable (a real asset - it will be reimbursed).
        - The company's share is additionally debited, as one lump sum for
          the whole order, to the configured discount expense account (a
          real, visible cost).
        ...
        - Net effect: each product's Sales line = full gross price before any
          discount or tax. Total reported profit is unchanged; only Sales
          presentation changes, and the company's cost becomes a visible
          expense line instead of being invisibly netted into Sales.
        """
        ...
        for line in lines:
            ...
            if has_aggregator_contribution or has_company_contribution:
                income_account = (
                ...
                if not income_account:
                    raise UserError(_(
                        ...
                        "the online campaign discount gross-up.",
            ...
            if has_aggregator_contribution:
                ...
                sales_topup[(income_account, line.product_id)] += (
                    direction * line.aggregator_contribution_amount
                )
        ...
        debit_specs = [
            ...
            (account, amount, _("Company contribution"), "product")
        ...
        self._apply_sales_topup(sales_topup)

        return data

    def _apply_sales_topup(self, sales_topup):
        """Increase the credit on each product's own core Sales line by the
        combined aggregator + company contribution amounts, instead of
        ...
        product_id and top up its credit - so both campaign shares are
        folded directly into the items sold, proportional to each item's
        own discount, grossing Sales back up to the full pre-discount,
        pre-tax price. Needs no manual maintenance as items or discount
        percentages change.
        """
        for (account, product), amount in sales_topup.items():
            ...
            if not target_lines:
                raise UserError(_(
                    ...
                    "the online campaign discount gross-up. Configure the module's income "
