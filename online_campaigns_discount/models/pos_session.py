...
class PosSession(models.Model):
    ...
    def _create_non_reconciliable_move_lines(self, data):
        ...
        for line in lines:
            ...
            is_aggregator_contribution = not float_is_zero(
                line.aggregator_contribution_amount, precision_rounding=self.currency_id.rounding
            )
            if is_aggregator_contribution and not aggregator.receivable_account_id:
                raise UserError(_(
                    "Configure receivable account on aggregator %s before closing the session because there is a non-zero aggregator contribution.",
                    aggregator.display_name,
                ))
