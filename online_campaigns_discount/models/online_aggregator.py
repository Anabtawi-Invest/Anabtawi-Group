...
class OnlineCampaignAggregator(models.Model):
    ...
    require_order_ref = fields.Boolean(
        string="Require Order ID at Payment",
        default=True,
        help="If enabled, the cashier must enter this aggregator's order number "
        "before validating payment on an order paid with one of its payment methods. "
        "Used to match POS orders against the aggregator statement during settlement.",
    )
    order_ref_digit_length = fields.Integer(
        string="Order ID Digit Length",
        default=10,
        help="Exact number of digits the aggregator's order number must have "
        "(e.g. Talabat order numbers are 10 digits). The POS payment screen "
        "rejects entries that don't match this length.",
    )
