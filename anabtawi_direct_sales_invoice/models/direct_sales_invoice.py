...
class DirectSalesInvoice(models.Model):
    ...
    def action_confirm_receiving(self):
        """Action for Salesperson / Cash Van driver to confirm receiving stock into their possession."""
        for record in self:
            if record.state not in ("warehouse_approved", "partially_approved", "ready"):
                raise UserError(_("Receiving can only be confirmed after warehouse approval."))
            pickings_to_validate = record.picking_ids.filtered(
    ...
    def action_send_email(self):
        ...
        ctx = {
            "default_model": "direct.sales.invoice",
    ...
    def action_print_pdf(self):
        """Trigger PDF Report Download for the Direct Invoice."""
        self.ensure_one()
        return self.env.ref("anabtawi_direct_sales_invoice.action_report_goods_release").report_action(self)
