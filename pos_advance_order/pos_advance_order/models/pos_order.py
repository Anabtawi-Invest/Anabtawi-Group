...
class PosOrder(models.Model):
    ...
    def _include_in_pledge_closing_summary(self):
        """Site Service advance orders must not affect pledge closing lines or cash balance."""
        self.ensure_one()
        advance = self.advance_order_id
        if advance and advance.site_service:
            return False
        if advance and hasattr(advance, "_pledge_applies") and not advance._pledge_applies():
            return False
        if self.advance_pledge_line_ids.filtered("order_id.site_service"):
            return False
        return True

    def _get_pledge_closing_amount(self):
        """Pledge amount to include in session closing (0 for Site Service advances)."""
        self.ensure_one()
        if not self._include_in_pledge_closing_summary():
            return 0.0
        pledge_total, _pids, pledge_qty = self._get_pledge_totals()
        if pledge_qty <= 0 or (self.currency_id or self.company_id.currency_id).is_zero(pledge_total):
            return 0.0
        return pledge_total
