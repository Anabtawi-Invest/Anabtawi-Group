...
class PosPaymentMethod(models.Model):
    ...
    def _get_payment_terminal_selection(self):
        # Standard Odoo extension point (see pos_adyen/pos_stripe/...): this is how a
        # payment method becomes selectable under Integration > Terminal > "Integrate with".
        return super()._get_payment_terminal_selection() + [("meps", "MEPS")]
    ...
    def meps_sale(self, amount, invoice_number="", reference_number=""):
        ...
        Called from the frontend PaymentInterface (static/src/app/utils/payment/payment_meps.js)
        via this.pos.data.call(...) - the standard mechanism used by every Odoo payment terminal.
        """
        self.ensure_one()
        self._meps_check_access()
        if self.use_payment_terminal != "meps":
