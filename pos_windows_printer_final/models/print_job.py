    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("printer_id"):
                printer = self.env["printer.printer"].browse(vals["printer_id"])
                if printer.exists() and printer.print_engine_client_id:
                    if not vals.get("print_engine_client_id"):
                        vals["print_engine_client_id"] = printer.print_engine_client_id.id
                    if not vals.get("print_engine_key"):
                        vals["print_engine_key"] = printer.print_engine_client_id.print_engine_key
                if not vals.get("printer_name") and printer.name:
                    vals["printer_name"] = printer.name
        return super(PrintJob, self).create(vals_list)
