...
patch(PosStore.prototype, {
    async processServerData(loadedData) {
        ...
        if (this.config) {
            if (this.config.use_pricelist === undefined) {
                this.config.use_pricelist = false;
            }
            if (!this.config.trusted_config_ids || !Array.isArray(this.config.trusted_config_ids)) {
                this.config.trusted_config_ids = [];
            }
            if (this.config.raw) {
                if (this.config.raw.use_pricelist === undefined) {
                    this.config.raw.use_pricelist = false;
                }
                if (!this.config.raw.trusted_config_ids || !Array.isArray(this.config.raw.trusted_config_ids)) {
                    this.config.raw.trusted_config_ids = [];
                }
            }
            if (this.config.direct_printer_id) {
