...
patch(PosStore.prototype, {
    async setup() {
        ...
        if (this.company) {
            ...
            if (!this.company.currency_id && this.currency) {
                this.company.currency_id = this.currency;
            }
        }
        if (this.config) {
            if (!this.config.trusted_config_ids || !Array.isArray(this.config.trusted_config_ids)) {
                this.config.trusted_config_ids = [];
            }
            if (this.config.raw && (!this.config.raw.trusted_config_ids || !Array.isArray(this.config.raw.trusted_config_ids))) {
    ...
    afterProcessServerData() {
        ...
        return super.afterProcessServerData(...arguments).then(function () {
            if (self.company) {
                if (!self.company.tax_calculation_rounding_method) {
                    self.company.tax_calculation_rounding_method = 'round_per_line';
                }
                if (!self.company.currency_id && self.currency) {
                    self.company.currency_id = self.currency;
                }
            }
            if (self.config) {
                if (!self.config.trusted_config_ids || !Array.isArray(self.config.trusted_config_ids)) {
                    self.config.trusted_config_ids = [];
                }
                if (self.config.raw && (!self.config.raw.trusted_config_ids || !Array.isArray(self.config.raw.trusted_config_ids))) {
                    self.config.raw.trusted_config_ids = [];
                }
                if (self.config.direct_printer_id) {
