...
patch(PosStore.prototype, {
    ...
    async processServerData(loadedData) {
        if (loadedData) {
            ...
            if (!defaultCurrency) {
                defaultCurrency = { id: 1, name: "USD", symbol: "$", rounding: 0.01, decimal_places: 2 };
                if (!loadedData["res.currency"]) {
                    loadedData["res.currency"] = [defaultCurrency];
                }
            }
            ...
            if (loadedData["res.company"]) {
                ...
                for (const comp of companies) {
                    ...
                    if (!comp.currency_id) {
                        comp.currency_id = [defaultCurrencyId, defaultCurrency.name || "USD"];
                    }
                }
            }
            ...
            if (loadedData["pos.config"]) {
                ...
                for (const cfg of configs) {
                    ...
                    if (!cfg.currency_id) {
                        cfg.currency_id = [defaultCurrencyId, defaultCurrency.name || "USD"];
                    }
                }
            }
        }
    ...
    _ensureValidCurrency() {
        if (!this.currency) {
            const firstCurrency = this.models && this.models["res.currency"] ? this.models["res.currency"].getAll()[0] : null;
            this.currency = firstCurrency || { id: 1, name: "USD", symbol: "$", rounding: 0.01, decimal_places: 2 };
        }
        if (this.company) {
            ...
            if (!this.company.currency_id) {
                this.company.currency_id = this.currency.id ? [this.currency.id, this.currency.name] : this.currency;
            }
        }
    },
...
patch(PosOrder.prototype, {
    get currency() {
        const curr = super.currency;
        if (curr) {
            return curr;
        }
        return this.pos && this.pos.currency ? this.pos.currency : { id: 1, name: "USD", symbol: "$", rounding: 0.01, decimal_places: 2 };
    }
});
