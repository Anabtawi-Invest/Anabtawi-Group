...
export class PosDirectPrinter extends BasePrinter {
    ...
    async printViaWebBluetooth(img) {
        try {
            ...
            if (characteristics.length > 0) {
                ...
                server.disconnect();
                console.warn("[PosDirectPrinter] Bluetooth device connected, but raster image transport is not supported.");
            }
        } catch (err) {
    ...
    async openCashbox() {
        ...
        const ip = typeof this.printer_id === "object" ? this.printer_id.ip : "";
        const port = typeof this.printer_id === "object" ? (parseInt(this.printer_id.port) || 80) : 80;
        const pType = typeof this.printer_id === "object" ? this.printer_id.printer_type : "";
        ...
        if (typeof this.printer_id === "object" && this.printer_id.print_engine_key) {
            try {
        ...
        return { result: false, printerErrorCode: "No cash drawer connection is configured" };
    }
    ...
    async sendPrintingJob(img) {
        ...
        const printerId = typeof this.printer_id === "object" ? this.printer_id.id : this.printer_id;
        const printerName = typeof this.printer_id === "object" ? (this.printer_id.name || '') : '';
        const printEngineKey = typeof this.printer_id === "object" ? (this.printer_id.print_engine_key || '') : '';
        const ip = typeof this.printer_id === "object" ? this.printer_id.ip : '';
        const port = typeof this.printer_id === "object" ? (parseInt(this.printer_id.port) || 80) : 80;
        const pType = typeof this.printer_id === "object" ? (this.printer_id.printer_type || 'image') : 'image';
        const cleanImageData = typeof img === "string"
        ...
        if (printEngineKey && (typeof navigator === "undefined" || navigator.onLine)) {
            try {
                const result = await rpc('/web/dataset/call_kw', {
                    model: 'print.job',
                    method: 'create',
                    args: [{
                        ...
                        print_engine_key: printEngineKey,
                        print_type: pType === 'raw' ? 'raw' : 'image',
