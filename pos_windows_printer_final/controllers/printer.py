...
class WindowsPrinterAgentController(http.Controller):
    ...
    def verify_api_token(self, **kwargs):
        ...
        try:
            ...
            if not params and hasattr(request, "dispatcher") and hasattr(request.dispatcher, "jsonrequest"):
                params = request.dispatcher.jsonrequest.get("params", {})

            printers_data = params.get("printers") or kwargs.get("printers", [])
            api_token = request.httprequest.headers.get("Authorization")

            if api_token:
                PrintEngineClient = request.env["print.engine.client"].sudo()
                client = PrintEngineClient.search([("print_engine_key", "=", api_token)], limit=1)
                if client and printers_data:
            ...
            return {"status": "error", "message": message, "sync_error": str(e)}
    ...
    def update_job_status(self, **kwargs):
        ...
        try:
            ...
            jobs = params.get("jobs") or kwargs.get("jobs", [])
            if not jobs:
