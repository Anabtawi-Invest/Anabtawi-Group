...
COLUMNS = [
    ('branch_name', 'Branch Name / اسم الفرع', 34, 'text'),
...
def build_retail_labor_cost_xlsx(rows, metadata):
    ...
    kpi_val_margin = book.add_format(dict(kpi_box, bold=True, font_size=12, bg_color='#F0FDF4', font_color='#15803D', num_format='#,##0.000'))
    ...
    # Header section
    sheet.merge_range('A1:J1', 'RETAIL LABOR COST & SALES PROFIT REPORT | تقرير تكلفة عمالة وأرباح أفرع الريتيل', title_fmt)
    sheet.merge_range('A2:J2', f"Company: {metadata.get('company', '')}  |  Period: {metadata.get('period', '')}", subtitle_fmt)
    sheet.merge_range('A3:J3', f"Generated by: {metadata.get('user', '')}  |  Date: {metadata.get('generated', '')}  |  Currency: {metadata.get('currency', 'JOD')}", info_fmt)
    ...
    tot_margin = tot_sales - tot_cost

    sheet.merge_range('A4:B4', 'TOTAL SALES / إجمالي مبيعات الأفرع', kpi_title_fmt)
    sheet.merge_range('A5:B5', tot_sales, kpi_val_sales)

    sheet.merge_range('C4:D4', 'TOTAL LABOR COST / إجمالي تكلفة الموظفين', kpi_title_fmt)
    sheet.merge_range('C5:D5', tot_cost, kpi_val_cost)

    sheet.merge_range('E4:F4', 'APPROVED OT / أوفر تايم معتمد', kpi_title_fmt)
    sheet.merge_range('E5:F5', f'{tot_ot_app:,.2f} Hrs', kpi_val_ot_app)

    sheet.merge_range('G4:H4', 'UNAPPROVED OT / أوفر تايم غير معتمد', kpi_title_fmt)
    sheet.merge_range('G5:H5', f'{tot_ot_unapp:,.2f} Hrs', kpi_val_ot_unapp)

    sheet.merge_range('I4:J4', 'NET MARGIN / صافي الهامش', kpi_title_fmt)
    sheet.merge_range('I5:J5', tot_margin, kpi_val_margin)

    sheet.set_row(3, 20)
    sheet.set_row(4, 26)

    # Table Header (Row 7, index 6)
    header_row = 6
    ...
    for r_idx, row_data in enumerate(rows, first_data_row):
        ...
        for c_idx, (key, _, _, kind) in enumerate(COLUMNS):
            ...
            if val is None or val == '':
                ...
                sheet.write_string(r_idx, c_idx, str(val), text_fmt)
            elif kind == 'integer':
    ...
    sheet.write(totals_row, 0, 'TOTAL / الإجمالي العام', total_label_fmt)
    ...
    for c_idx, (key, _, _, kind) in enumerate(COLUMNS):
        ...
        if kind == 'integer':
            ...
            sales_col = xlsxwriter.utility.xl_col_to_name(1)  # Column B
            cost_col = xlsxwriter.utility.xl_col_to_name(4)   # Column E
