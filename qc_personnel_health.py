...
class QcInspection(models.Model):
    _name = "qc.inspection"
    _description = "Site Quality Inspection"
    ...
    reset_reason = fields.Char(
        string="Reset Reason", copy=False,
        help="Mandatory justification when resetting an Approved or Closed "
             "inspection back to Scheduled. Posted permanently to the log.",
    )
    ...
    def action_reset_to_scheduled(self):
        ...
        for inspection in self:
            if inspection.state in ("approved", "closed") \
                    and not inspection.reset_reason:
                raise UserError(_(
                    "Enter a reason before resetting an Approved or Closed "
                    "inspection back to Scheduled. It will be recorded "
                    "permanently in the log."))
            if inspection.reset_reason:
                inspection.message_post(body=_(
                    "Inspection reset to Scheduled by %(user)s from state "
                    "'%(state)s'. Reason: %(reason)s") % {
                    "user": self.env.user.name,
                    "state": inspection.state,
                    "reason": inspection.reset_reason,
                })
        self.write({"state": "scheduled", "reset_reason": False})
        return True
    ...
    def _create_quality_alert(self):
        ...
        param = self.env["ir.config_parameter"].sudo().get_param(
            "site_quality_control.create_quality_alert", default="True")
