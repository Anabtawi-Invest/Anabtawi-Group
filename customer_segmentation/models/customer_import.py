...
class CustomerImport(models.Model):
    ...
    def action_validate_file(self):
        ...
        try:
            ...
            # Update import record (replace previous validation lines)
            self.write({'import_lines': [(5, 0, 0)] + import_lines})
    ...
    def action_import_customers(self):
        ...
        for line in self.import_lines:
            ...
            try:
                duplicate_domain = [('mobile', '=', line.mobile)]
                if line.email:
                    duplicate_domain = ['|'] + duplicate_domain + [('email', '=', line.email)]
                existing = Partner.search(duplicate_domain, limit=1)
                ...
                # Set department-specific flags and partner ranking
