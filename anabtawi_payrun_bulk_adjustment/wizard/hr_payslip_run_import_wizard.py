...
class HrPayslipRunImportWizard(models.TransientModel):
    ...
    payrun_id = fields.Many2one('hr.payslip.run', string="Payrun", required=True, ondelete='cascade')
    ...
    def _get_slip_actual_salary(self, slip):
        ...
        # Fallbacks if lines are not computed yet
    ...
    def action_export_template(self):
        ...
        # Populate rows with employees in this Payrun
        slips = self.payrun_id.slip_ids.sorted(key=lambda s: s.employee_id.name or '')
        for slip in slips:
            ...
            # Check existing partial payment input
        ...
        filename = f"Partial_Salary_Payment_{self.payrun_id.name or 'Draft'}.xlsx".replace(" ", "_")
    ...
    def action_import_adjustments(self):
        ...
        if partial_col_idx is None:
            # Fallback to column index 3 if column 4 in 1-based indexing
            partial_col_idx = 3 if len(header_row) > 3 else 2

        partial_input_type = self._get_partial_payment_input_type()

        # Map slips by code and ID
        slips_by_code = {}
