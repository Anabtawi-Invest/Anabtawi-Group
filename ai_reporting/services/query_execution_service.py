...
class AiReportingQueryExecutionService(models.AbstractModel):
    ...
    def execute_plan(self, plan, parameters=None, preview=False):
        ...
        if groupby and measures:
            result = self._read_group(model, domain, groupby, measures, limit)
        else:
            result = {
    ...
    def _read_group(self, model, domain, groupby, measures, limit):
        fields_spec = []
        ...
        rows = model.read_group(domain, fields_spec, groupby, limit=limit, lazy=False)
