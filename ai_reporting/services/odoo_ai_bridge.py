...
class AiReportingOdooAiBridge(models.AbstractModel):
    ...
    def detect_native_ai(self):
        ...
        if status["oca_available"]:
            status["provider_configured"] = bool(self._get_oca_connection())
    ...
    def ask_native_provider(self, question, parameters=None):
        ...
        if not status["available"]:
            return {
                ...
                "answer": _("Native Odoo AI is not available in this environment."),
                "question": question,
                "parameters": parameters or {},
            }
        if status.get("oca_available") and self._get_oca_connection():
            answer = self._run_oca_connection(
                prompt=question,
                system_prompt=self._ask_ai_system_prompt(),
            )
            return {
                "provider_called": True,
                "provider_name": "oca_ai_connection",
        ...
        return {
            "provider_called": False,
            "answer": _("Native AI provider integration point detected. Configure the bridge for the installed Enterprise AI provider."),
            "status": status,
        }

    def create_report_draft(self, question):
        if self._get_oca_connection():
            oca_result = self._run_oca_connection(
                prompt=question,
                system_prompt=self._advanced_report_system_prompt(),
            )
            parsed = self._parse_report_definition(oca_result.get("content"))
            if parsed:
                return parsed
    ...
    def adjust_report_draft(self, definition, adjustment):
        if self._get_oca_connection():
            oca_result = self._run_oca_connection(
                prompt=self._adjustment_prompt(definition, adjustment),
                system_prompt=self._advanced_report_system_prompt(),
            )
            parsed = self._parse_report_definition(oca_result.get("content"))
            if parsed:
