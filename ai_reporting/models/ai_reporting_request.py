...
class AiReportingRequest(models.Model):
    ...
    # See services/optional_ai_tool.py. This is the same draft-and-preview
    # flow as the built-in chatbot, just callable by an external MCP client
    # or automation. It never saves anything -- confirmation still has to
    # happen in Odoo, same as section 18 of the design requires for the
    # chatbot itself.
    ...
    def _ai_tool_create_advanced_report(self, question):
        """Draft and preview an Advanced Report from a natural-language
        question, exactly like the chatbot. Nothing is saved until a human
        confirms it in Odoo (action_confirm + action_save_report)."""
        request = self.create({"question": question})
        request.action_generate_draft()
        return {
            "request_id": request.id,
            "state": request.state,
            "preview": request.preview_result_metadata_json,
        }
