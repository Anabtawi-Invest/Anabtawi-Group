...
export class AiReportingReportBuilder extends Component {
    ...
    async createDraft() {
        ...
        try {
            const result = await this.rpc("/ai_reporting/report_builder/create", { question });
    ...
    async adjustDraft() {
        ...
        this.state.busy = true;
        try {
            const result = await this.rpc("/ai_reporting/report_builder/adjust", {
    ...
    async confirmDraft() {
        ...
        this.state.busy = true;
        try {
            const result = await this.rpc("/ai_reporting/report_builder/confirm", {
                request_id: this.state.requestId,
