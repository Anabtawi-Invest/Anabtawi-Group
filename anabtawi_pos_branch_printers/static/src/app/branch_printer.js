...
export class BranchPrinter extends BasePrinter {
    ...
    async openCashbox() {
        try {
            ...
            if (res && res.error) {
                console.warn('[BranchPrinter] Cashbox signal error:', res.error);
            }
    ...
    async sendPrintingJob(img) {
        ...
        try {
            ...
            if (res && res.result) {
                console.log('[BranchPrinter] Direct thermal print succeeded.');
            } else {
                console.warn('[BranchPrinter] Network print unreachable or printer offline:', res ? res.error : 'No response');
            }
            // Always return result: true so POS completes sale seamlessly without IoT Box errors
            return { result: true, printerErrorCode: false };
        } catch (err) {
            console.error('[BranchPrinter] Print job RPC error:', err);
            return { result: true, printerErrorCode: false };
        }
    }
}
