...
| **FIX-18** | **Public Holiday 8-Hour Duration Normalization** | Full-day public holidays generated 24-hour work entries and 3-day payslip worked day lines. | Added `_normalize_public_holiday_work_entries()` to clamp PHD work entries to 8.0 hours during `compute_sheet()`, and updated `_get_worked_day_lines()` to cap unworked holiday lines to `len(holiday_dates) * 8.0` hours (1 day = 8.0h). | `models/hr_payslip.py` |
