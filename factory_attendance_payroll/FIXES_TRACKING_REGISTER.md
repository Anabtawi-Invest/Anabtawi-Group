...
| **FIX-11** | **Out of Contract Period & Grace Day Isolation** | Pre-contract start date days were returning `8.0h` expected hours, consuming monthly grace days and causing unworked day undertime deductions. | Updated `_get_expected_hours_on_day()` to return `0.0h` out-of-contract, and bounded payslip attendance reconciliation to active contract dates `[eff_date_from, eff_date_to]`. | `models/hr_employee.py`, `models/hr_payslip.py` |
