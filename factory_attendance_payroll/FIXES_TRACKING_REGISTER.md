...
| **FIX-17** | **Monday-Based Flexible Rest Day Quota** | Flexible schedule rest day quota relied on `worked // 6`, which truncated 5-Monday months to 4 rest days. | Updated `_convert_flexible_rest_days_to_ars()` to count Mondays (`weekday() == 0`) in the payslip period, granting 4 rest days in 4-Monday months and 5 rest days in 5-Monday months while allowing employees to choose any day of the week off. | `models/hr_payslip.py` |
