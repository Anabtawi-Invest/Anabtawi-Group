...
class HrAttendance(models.Model):
    ...
    def _compute_factory_attendance_metrics(self):
        for attendance in self:
            ...
            is_ph = attendance.employee_id._is_public_holiday_on_day(target_date) if (attendance.employee_id and target_date) else False
            if is_ph:
                standard_target = 0.0
            else:
            ...
            if excess >= min_ot_threshold or (is_ph and excess > 0.0):
                attendance.daily_overtime_hours = excess
                attendance.daily_undertime_hours = 0.0
                attendance.daily_variance_hours = excess
            elif excess < -min_lateness_threshold and not is_ph:
                attendance.daily_overtime_hours = 0.0
    ...
    def _compute_overtime_hours(self):
        ...
        for attendance in self:
            ...
            if is_ph:
                ...
                standard_target = expected_hrs if expected_hrs > 0 else 8.0
            excess = net_hrs - standard_target
            attendance.overtime_hours = excess if (excess >= 0.75 or (is_ph and excess > 0.0)) else 0.0
    ...
    def action_approve_factory_overtime(self):
        ...
        """
        for att in self:
            target_date = att.check_in.date() if att.check_in else None
            is_ph = att.employee_id._is_public_holiday_on_day(target_date) if (att.employee_id and target_date) else False
            min_th = 0.0 if is_ph else 0.75
            att_ot = att.daily_overtime_hours if att.daily_overtime_hours >= min_th and att.daily_overtime_hours > 0 else 0.0
            if att_ot <= 0:
    ...
    def _compute_eligible_overtime(self):
        ...
        Only attendance records with positive Daily Extra Hours >= 0.75h (or any positive hours on Public Holidays) are eligible for approval.
        """
        ...
        for attendance in self:
            target_date = attendance.check_in.date() if attendance.check_in else False
            is_ph = attendance.employee_id._is_public_holiday_on_day(target_date) if (attendance.employee_id and target_date) else False
            if attendance.daily_overtime_hours >= 0.75 or (is_ph and attendance.daily_overtime_hours > 0):
    ...
    def _check_weekly_overtime_eligibility(self):
        ...
        """
        factory_atts = self.filtered(lambda a: a.daily_overtime_hours >= 0.75 or (a.employee_id and a.check_in and a.employee_id._is_public_holiday_on_day(a.check_in.date()) and a.daily_overtime_hours > 0))
        ...
        for att in valid_atts:
            try:
                ...
                if is_ph:
                    ...
                    standard_target = expected_hrs if expected_hrs > 0 else 8.0
                excess = net_hrs - standard_target
                min_ot_threshold = 0.0 if is_ph else 0.75
                ...
                if excess < min_ot_threshold or excess <= 0:
                    if ot_rec:
