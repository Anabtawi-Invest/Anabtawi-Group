...
class HrPayslip(models.Model):
    ...
    def action_reconcile_lateness_no_ot_bank(self):
        ...
        for slip in self:
            ...
            if remaining > 0:
                leave_type_id = int(self.env['ir.config_parameter'].sudo().get_param('mass_reconciliation.annual_leave_type_id') or self.env['ir.config_parameter'].sudo().get_param('lateness_coverage.annual_leave_type_id') or 0)
                ...
                # Build leave values in a version-safe way (avoid number_of_hours_display which may not exist).
                dt_from = fields.Datetime.to_datetime(slip.date_from)
                dt_to = dt_from + timedelta(hours=remaining)
                ...
                # Optional request_* fields (safe across versions)
                ...
                # Safety guard: remove any unsupported fields for this Odoo build
