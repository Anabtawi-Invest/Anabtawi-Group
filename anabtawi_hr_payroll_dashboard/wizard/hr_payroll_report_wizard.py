...
class HrPayrollReportWizard(models.TransientModel):
    ...
    def action_export_xlsx(self):
        ...
        fill_primary_header = PatternFill(start_color="1E40AF", end_color="1E40AF", fill_type="solid") # Dark Blue
        fill_alw_header = PatternFill(start_color="047857", end_color="047857", fill_type="solid") # Emerald Green
        fill_ded_header = PatternFill(start_color="B91C1C", end_color="B91C1C", fill_type="solid") # Dark Red
        fill_sub_header = PatternFill(start_color="E2E8F0", end_color="E2E8F0", fill_type="solid") # Light Slate
        fill_group_total = PatternFill(start_color="F1F5F9", end_color="F1F5F9", fill_type="solid") # Group Total
        fill_grand_total = PatternFill(start_color="FEF08A", end_color="FEF08A", fill_type="solid") # Yellow Accent
        ...
        if self.report_type in ["all", "payroll"]:
            ...
            ws_pay.merge_range = False # reset flag
            ...
            for slip in payslips:
                ...
                # Basic / Contract
                basic_w = getattr(slip, "basic_wage", 0.0) or (slip.contract_id and slip.contract_id.wage) or (slip.version_id and getattr(slip.version_id, "contract_wage", 0.0)) or slip.wage or 0.0
                actual_w = basic_w
                due_w = basic_w

                # Allowances & Deductions from lines
                alw_takleef = 0.0
                alw_bonus = 0.0
                alw_ot = 0.0
                alw_trans = 0.0
        ...
        if self.report_type in ["all", "overtime"]:
            ...
            for slip in payslips:
                ...
                basic_w = getattr(slip, "basic_wage", 0.0) or slip.wage or 0.0
                pay_rate = round(basic_w / 240.0, 3) if basic_w else 0.0 # Hourly rate based on standard 240 hrs

                # Extract OT hours & amount
                ot_hours = 0.0
                ot_amt = 0.0
                if hasattr(slip, "worked_days_line_ids") and slip.worked_days_line_ids:
        ...
        if self.report_type in ["all", "bank"]:
            ...
            # Standard banking columns without top decorators (matching raw bank file)
            bank_row = 1
            ...
            for slip in payslips:
                ...
                # If IBAN starts with JO -> Local Bank Transfer (LBT), else intra-bank (BT)
        ...
        if self.report_type in ["all", "attendance"]:
            ...
            for slip in payslips:
                ...
                # Format as HH:MM string like in attendance recon file
                def _fmt_hrs(val):
                    sign = "-" if val < 0 else ""
                    val = abs(val)
                    h = int(val)
