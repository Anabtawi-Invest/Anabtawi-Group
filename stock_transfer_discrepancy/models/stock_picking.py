...
class StockPicking(models.Model):
    ...
    def _get_transfer_discrepancy_move_vals(self):
        ...
        # Only relevant when a truck is involved (either source or destination).
        if not (self.location_id.is_truck or self.location_dest_id.is_truck):
            _logger.warning(
                "[DISCREPANCY] No truck location found. Source is_truck=%s, Dest is_truck=%s",
                self.location_id.is_truck,
                self.location_dest_id.is_truck,
            )
            return []
        ...
        elif self.location_id.is_truck and self.location_dest_id.is_truck:
            # Both are trucks: use source location as truck_location
            truck_location = self.location_id
            stage = "dispatch"  # Default to dispatch when both are trucks
            _logger.info(
                "[DISCREPANCY] Both locations are trucks. Using SOURCE: %s (stage: dispatch)",
                truck_location.name,
            )
        else:
            # Should not reach here (already checked above)
            _logger.error(
                "[DISCREPANCY] Unexpected case: no truck found but passed initial check!"
            )
            return []
    ...
    def _action_done(self):
        ...
        # Settlement via return/internal transfers:
            ...
            if picking.location_dest_id.is_truck:
                ...
                for move in done_moves:
                    ...
                    Discrepancy.apply_resolution(
                        truck, move.product_id, qty_prod_uom, stage="dispatch", exclude_picking_ids=[picking.id]
                    )
    ...
    def _onchange_picking_type_id_discrepancy_truck_domain(self):
        ...
        _logger.info(
            "[DISCREPANCY DOMAIN] Applying domain to location_id and location_dest_id: %s",
            domain,
        )
        ...
        if self.location_dest_id:
            _logger.info(
                "[DISCREPANCY DOMAIN] Current location_dest_id: %s (ID: %s, is_truck: %s, has_open_discrepancy: %s)",
                self.location_dest_id.name,
                self.location_dest_id.id,
                self.location_dest_id.is_truck,
                self.location_dest_id.has_open_discrepancy,
            )
            if self.location_dest_id.is_truck and self.location_dest_id.has_open_discrepancy:
                if warning_msg:
                    warning_msg += "\n\n"
                else:
                    warning_msg = ""
                warning_msg += _(
                    "Cannot use truck location '%s' as destination location: it has open discrepancies. "
                    "Please settle the discrepancies first or choose another location.",
                    self.location_dest_id.name,
                )
                _logger.warning(
                    "[DISCREPANCY DOMAIN] Warning: Destination location %s has open discrepancy",
                    self.location_dest_id.name,
                )
                # Clear the location to force user to choose another one
                self.location_dest_id = False
        
        result = {"domain": {"location_id": domain, "location_dest_id": domain}}
        if warning_msg:
            result["warning"] = {
    ...
    def _check_truck_open_discrepancy(self):
        ...
        for picking in self:
            ...
            if picking.location_dest_id.is_truck and picking.location_dest_id.has_open_discrepancy:
                _logger.warning(
                    "[DISCREPANCY DOMAIN] Constraint violation: Destination location %s (ID: %s) is truck with open discrepancy",
