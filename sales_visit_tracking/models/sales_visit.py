...
class SalesVisit(models.Model):
    ...
    verification_status = fields.Selection([
        ('valid', 'Valid Visit - Under 100m'),
        ('warning', 'Warning - 101 to 300m'),
        ('invalid', 'Invalid Visit - Over 300m'),
    ...
    partner_id = fields.Many2one(
        ...
        copy=False,
        index=True,
        help='Populated when the lead is converted to a customer (Approved outcome).'
    )
    ...
    def action_start_visit(self, lead_id, latitude, longitude):
        ...
        allowed_radius = float(param) if param else 100.0
    ...
    def action_end_visit(self, latitude, longitude, result,
                         next_visit_date=None, rejection_reason=None):
        self.ensure_one()
    ...
    def get_dashboard_data(self):
        ...
        completed = visits_today.filtered(lambda v: v.check_out_time)
        ...
        missed = visits_today.filtered(lambda v: not v.check_out_time and v.check_in_time)

        total = len(visits_today)
        valid_gps = len(visits_today.filtered(lambda v: v.verification_status == 'valid'))
        gps_compliance = round(valid_gps / total * 100.0, 1) if total > 0 else 100.0

        return {
            'today_visits': total,
            'completed_visits': len(completed),
            'missed_visits': len(missed),
