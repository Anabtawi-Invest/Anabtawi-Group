...
class PosCustomCakeUploadController(http.Controller):
    ...
    def _get_session_by_code(self, code):
        return request.env["pos.cake.upload.session"].get_by_code(code)
    ...
    def _labels(self):
        ...
        return {
            ...
            "subtitle": _lt("Enter the code shown on the cashier screen, then upload your photos."),
            "code_label": _lt("Upload code"),
            "code_placeholder": _lt("Example: A3K9"),
            "continue": _lt("Continue"),
            "add_photos": _lt("Add photos"),
            "upload": _lt("Upload"),
            "delete": _lt("Delete"),
            "no_photos": _lt("No photos uploaded yet."),
            "expired": _lt("This upload code has expired."),
            "expired_help": _lt("Ask the cashier to open the cake form again and give you a new code."),
            "formats": _lt("JPG, PNG, WEBP or GIF. Maximum 8 MB per photo."),
            "count": _lt("Photos"),
            "code_invalid": _lt("Invalid or expired upload code. Please check the code on the cashier screen."),
            "change_code": _lt("Use another code"),
            "code_hint": _lt("Ask the cashier for the 4-character code."),
        }, ("ar" if is_rtl else "en"), ("rtl" if is_rtl else "ltr")

    def _page_values(self, session=None, error=None, success=None, entered_code=""):
        labels, lang, direction = self._labels()
        return {
            "request": request,
            "session": session,
            ...
            "code": session.code if session else entered_code,
            "image_count": len(session.image_ids) if session else 0,
        }

    def _render_page(self, session=None, error=None, success=None, entered_code=""):
    ...
    def upload_entry(self, **post):
        """Fixed QR landing page: customer enters the short upload code."""
        if request.httprequest.method == "POST":
            code = post.get("code") or ""
            session = self._get_session_by_code(code)
            if not session or not session.is_valid:
                return self._render_page(
                    error=_("Invalid or expired upload code. Please check the code on the cashier screen."),
                    entered_code=code,
                )
            return request.redirect(f"/pos/custom_cake/upload/{session.token}")
        return self._render_page()
    ...
    def upload_page(self, token, **post):
        ...
        if not session:
            return self._render_page(error=_("Invalid or expired upload code. Please check the code on the cashier screen."))
        if request.httprequest.method == "POST":
