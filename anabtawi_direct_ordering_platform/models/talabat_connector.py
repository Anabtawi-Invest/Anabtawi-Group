...
class AnabtawiTalabatConfig(models.Model):
    ...
    auto_accept_orders = fields.Boolean(default=False, string='Auto Accept Talabat Orders')
    auto_accept_only_if_stock_available = fields.Boolean(default=True, string='Auto Accept Only If Stock Available')
    keep_needs_review_if_problem = fields.Boolean(default=True, string='Keep Needs Review If Problem')
    auto_print_after_accept = fields.Boolean(default=True, string='Auto Print After Auto Accept')
    notify_cashier_after_accept = fields.Boolean(default=True, string='Notify Cashier After Auto Accept')
    create_sale_order_on_import = fields.Boolean(default=False)

    inventory_sync_enabled = fields.Boolean(default=True, string='Sync Product Availability to Talabat')
    inventory_sync_interval_minutes = fields.Integer(default=30, string='Inventory Sync Interval (Minutes)')
    inventory_min_qty = fields.Float(default=1.0, string='Minimum Stock Qty')
    inventory_hide_if_below_min = fields.Boolean(default=True, string='Hide Item If Stock <= Minimum Qty')
    inventory_restore_if_above_min = fields.Boolean(default=True, string='Show Item Again If Stock > Minimum Qty')
    availability_endpoint = fields.Char(default='/products/availability', string='Availability Endpoint')
    last_inventory_sync = fields.Datetime(readonly=True)

    talabat_pro_fee_enabled = fields.Boolean(default=False, string='Enable Talabat Pro Fee')
    talabat_pro_indicator_fields = fields.Char(
        default='is_pro,talabat_pro,pro_customer,customer.is_pro,customer.talabat_pro,subscription.type',
        string='Talabat Pro Indicator Fields',
        help='Comma-separated payload paths. If any path is truthy or equals PRO, the order is treated as Talabat Pro.'
    )
    talabat_pro_fee_amount = fields.Monetary(default=0.6, string='Talabat Pro Fee')
    talabat_pro_fee_account_id = fields.Many2one('account.account', string='Talabat Pro Expense Account')
    ...
    def _create_or_update_order_from_payload(self, payload):
        ...
        if self.auto_accept_orders:
            if (not self.auto_accept_only_if_stock_available) or order._has_available_stock():
                order.action_accept()
                order.action_push_talabat_status()
            elif self.keep_needs_review_if_problem:
                order.write({'state': 'needs_review', 'talabat_sync_message': _('Auto accept skipped: insufficient stock or missing stock configuration.')})
                order._create_staff_notifications('new_order')
    ...
    def _prepare_direct_order_vals(self, payload):
        ...
        is_pro = self._payload_has_talabat_pro_indicator(payload)
        return {
            'channel': 'talabat',
            ...
            'talabat_is_pro_order': bool(is_pro),
            'talabat_pro_fee_amount': self.talabat_pro_fee_amount if (is_pro and self.talabat_pro_fee_enabled) else 0.0,
            'line_ids': lines,
        }


    def _payload_get_path(self, payload, path):
        value = payload
    ...
    def _payload_has_talabat_pro_indicator(self, payload):
        self.ensure_one()
        for path in (self.talabat_pro_indicator_fields or '').split(','):
            value = self._payload_get_path(payload, path.strip())
            if isinstance(value, str) and value.strip().upper() in ('PRO', 'TALABAT_PRO', 'TRUE', 'YES', '1'):
                return True
    ...
    @api.model
    def _cron_sync_inventory_availability(self):
        now = fields.Datetime.now()
        configs = self.search([('active', '=', True), ('inventory_sync_enabled', '=', True)])
        for config in configs:
            interval = max(config.inventory_sync_interval_minutes or 30, 1)
            if config.last_inventory_sync and config.last_inventory_sync > fields.Datetime.subtract(now, minutes=interval):
                continue
            try:
                config.action_sync_inventory_availability()
            except Exception as exc:
                _logger.exception('Talabat inventory availability sync failed for %s: %s', config.display_name, exc)
        return True

    def action_sync_inventory_availability(self):
        for config in self:
            branch_maps = self.env['anabtawi.talabat.branch.map'].sudo().search([
                ('config_id', '=', config.id),
                ('active', '=', True),
            ])
            product_maps = self.env['anabtawi.talabat.product.map'].sudo().search([
                ('config_id', '=', config.id),
                ('active', '=', True),
                ('sync_availability', '=', True),
            ])
            for branch_map in branch_maps:
                location = branch_map.branch_id.stock_location_id or branch_map.branch_id.warehouse_id.lot_stock_id
                if not location:
                    continue
                for product_map in product_maps:
                    product = product_map.product_id.with_context(location=location.id)
                    qty = product.qty_available
                    available = qty > (config.inventory_min_qty or 0.0)
                    if not config.inventory_hide_if_below_min and not available:
                        continue
                    if not config.inventory_restore_if_above_min and available:
                        continue
                    payload = {
                        'vendor_id': branch_map.talabat_vendor_id,
                        'item_id': product_map.talabat_item_id,
                        'sku': product_map.talabat_sku,
                        'available': bool(available),
                        'quantity': qty,
                    }
                    config._request('POST', config.availability_endpoint or '/products/availability', payload=payload)
                    product_map.write({
                        'last_available': bool(available),
                        'last_available_qty': qty,
...
class AnabtawiTalabatProductMap(models.Model):
    ...
    last_available = fields.Boolean(readonly=True)
    last_available_qty = fields.Float(readonly=True)
    last_sync_datetime = fields.Datetime(readonly=True)
