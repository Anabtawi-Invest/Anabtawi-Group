...
class PosPaymentSummaryWizard(models.TransientModel):
    ...
    def _get_report_lines(self):
        """Return report rows grouped by session opening date, POS, and payment method."""
        self.ensure_one()
    ...
    def _generate_xlsx_content(self):
        ...
        date_style = workbook.add_format({'border': 1, 'num_format': 'yyyy-mm-dd hh:mm'})
        ...
        if not lines:
            ...
            for line in lines:
                ...
                if opening_date:
                    sheet.write_datetime(row, 0, fields.Datetime.to_datetime(opening_date), date_style)
                else:
