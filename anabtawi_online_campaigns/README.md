...
## 19.0.2.0.4
Force upgrade package for report model registry refresh. Includes online.campaign.performance.report SQL model import.


## v19.0.2.0.4 Cap Options Update

Cap Application now supports only two options:
- Per Order Cap: one cap shared across all eligible discounted lines in the order. Non-eligible items do not consume the cap. The final eligible line is reduced to the remaining available cap.
- Per Line Cap: each eligible line has its own maximum discount cap. Non-eligible items are ignored.

Removed the old Per Unit cap option from the campaign form, POS line metadata, and POS calculation logic.
