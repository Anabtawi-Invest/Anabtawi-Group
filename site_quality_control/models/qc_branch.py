...
class QcBranch(models.Model):
    _name = "qc.branch"
    _description = "Quality Control Site"
    _inherit = ["mail.thread"]
    _order = "sequence, name"

    name = fields.Char(string="Site Name", required=True, tracking=True, translate=True)
    code = fields.Char(string="Code", tracking=True)
    sequence = fields.Integer(default=10)
    region = fields.Char(string="Region", tracking=True)
    manager_id = fields.Many2one(
        "res.users", string="Site/Dept Manager", tracking=True,
    )
    inspector_id = fields.Many2one(
        "res.users", string="Site Inspector", tracking=True,
    )
    ...
    min_passing_score = fields.Float(
        string="Minimum Passing Score", default=lambda self: self._default_min_passing_score(),
        help="Total score (out of 100) below which the site is considered failing.",
    )
    ...
    @api.model
    def _default_min_passing_score(self):
        param = self.env["ir.config_parameter"].sudo().get_param(
            "site_quality_control.default_passing_score", default="60.0")
        try:
            return float(param)
