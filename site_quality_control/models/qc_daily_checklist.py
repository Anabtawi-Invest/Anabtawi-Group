...
class QcDailyChecklist(models.Model):
    ...
    round = fields.Selection(
        ...
        string="Round", default="morning", required=True, tracking=True,
        help="Monitoring round of the day. Templates with several rounds per "
             "day generate one checklist per round.",
    )
    ...
    unlock_reason = fields.Char(
        ...
        help="Mandatory justification when a Quality Administrator unlocks a "
             "completed checklist. Posted permanently to the log.",
    )
    ...
    _sql_constraints = [
        ("branch_date_template_round_uniq",
         "unique(branch_id, date, template_id, round, company_id)",
         "A daily checklist already exists for this site, template, date "
         "and round."),
    ]
    ...
    def action_done(self):
        for checklist in self:
            ...
            # Measures taken with an overdue instrument are invalid.
            bad = checklist.line_ids.filtered(
                lambda l: l.answer_type == "measure" and l.instrument_id
                and not l.instrument_id._is_usable()).mapped("instrument_id")
            if bad:
                raise UserError(_(
                    "The following instruments are overdue for calibration "
                    "and cannot be used: %s") % ", ".join(bad.mapped("name")))
            checklist.write({
                "state": "done",
    ...
    def action_unlock(self):
        """Completed records are locked. Only a Quality Administrator may
        unlock them, with a mandatory reason kept in the chatter forever."""
        if not self.env.user.has_group(
                "site_quality_control.group_qc_admin"):
            raise UserError(_(
                "Completed checklists are locked. Only a Quality "
                "Administrator can unlock them."))
        for checklist in self:
            if checklist.state != "done":
                raise UserError(_("Only completed checklists can be unlocked."))
            if not checklist.unlock_reason:
                raise UserError(_(
                    "Enter an unlock reason before unlocking. It will be "
                    "recorded permanently in the log."))
            checklist.message_post(body=_(
                "Checklist unlocked by %(user)s. Reason: %(reason)s") % {
                "user": self.env.user.name,
                "reason": checklist.unlock_reason,
            })
            checklist.write({
                "state": "todo",
                "done_datetime": False,
                "unlock_count": checklist.unlock_count + 1,
                "unlock_reason": False,
            })
        return True
    ...
    def _cron_generate_daily_checklists(self):
        """Create today's checklist(s) for every active site, for every
        active daily template of its company — one per monitoring round."""
        ...
        for branch in branches:
            ...
            for template in branch_templates:
                rounds = rounds_map.get(template.daily_rounds or "1",
                                        ["morning"])
                for round_key in rounds:
                    ...
                    if branch.manager_id:
                        checklist.activity_schedule(
                            ...
                            summary=_("Fill today's site checklist (%s)")
                            % round_key,
                        )
        return True
...
class QcDailyChecklistLine(models.Model):
    ...
    instrument_id = fields.Many2one(
        ...
        help="Measuring instrument used for this measure.",
    )
