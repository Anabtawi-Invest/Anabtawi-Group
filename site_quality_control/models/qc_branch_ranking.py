...
class QcBranchRanking(models.Model):
    """Read-only SQL view: one row per site with its latest inspection,
    previous score, deltas and open/overdue corrective action counts."""
    _name = "qc.branch.ranking"
    _description = "Site Quality Ranking"
    _auto = False
    _order = "current_score desc"

    branch_id = fields.Many2one("qc.branch", string="Site", readonly=True)
