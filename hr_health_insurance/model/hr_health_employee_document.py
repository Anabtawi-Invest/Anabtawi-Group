...
class HrHealthEmployeeDocument(models.Model):
    ...
    def _get_before_expiry_days(self):
        before_days = self.env.company.before_the_expiry_date
        try:
            return max(int(before_days or 0), 0)
        except (TypeError, ValueError):
            _logger.warning("Invalid before_the_expiry_date value: %s", before_days)
            return 0
